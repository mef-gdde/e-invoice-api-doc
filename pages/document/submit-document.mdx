...
| document_type  | String   | The [type of document](/document-type) being submitted.                |
...
```json
{
    ...
    "verification_link": "https://caminv-dev-merc.mef.gov.kh/verification?data=df5dcf78-e78b-4396-87bb-2d2a44c1302e"
}
```
...
## Notes
- The endpoint allows submitting one or multiple documents for processing.
- If only one document is submitted, the response is synchronous and returns the document ID and verification link immediately.
- If multiple documents are submitted, the processing may be asynchronous, and the results will be sent via webhook.
```

This documentation provides details about the `/api/v1/document` endpoint, including its method, request body parameters, and possible responses. It covers both synchronous and asynchronous processing scenarios.